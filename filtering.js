    ...
    Promise.all(filterFetchPromises).then(function (fetchResults) {
      fetchResults.forEach(function (filtersForWorksheet) {
        filtersForWorksheet.forEach(function (filter) {
          console.log(filtersForWorksheet, filter)
          dashboardfilters.push(filter);
          artyom.addCommands({
            indexes:["set " + filter.fieldName + " to *value"], // These spoken words will trigger the execution of the command
            action:function(idx, value){ // Action to be executed when a index match with spoken word
              console.log(value);
            }
          })
        });
